...
namespace UnityEngine.XR.Interaction.Toolkit.Samples.DeviceSimulator
{
    class XRDeviceSimulatorUI : MonoBehaviour
    {
        ...
        void Initialize()
        {
            ...
            string headsetMoveBinding = "WASD";
            if (m_Simulator.axis2DAction != null && m_Simulator.axis2DAction.action != null)
            {
                var ctrlsBinding1 = m_Simulator.axis2DAction.action.controls;
                if (ctrlsBinding1.Count >= 4)
                    headsetMoveBinding = $"{ctrlsBinding1[0].displayName},{ctrlsBinding1[1].displayName},{ctrlsBinding1[2].displayName},{ctrlsBinding1[3].displayName}";
            }
            if (m_Simulator.keyboardYTranslateAction != null && m_Simulator.keyboardYTranslateAction.action != null)
            {
                var ctrlsBinding2 = m_Simulator.keyboardYTranslateAction.action.controls;
                if (ctrlsBinding2.Count >= 2)
                    headsetMoveBinding += $" + {ctrlsBinding2[0].displayName},{ctrlsBinding2[1].displayName}";
            }
            m_HeadsetMoveValueText.text = headsetMoveBinding;
        ...
        void PushCurrentButtonState(XRDeviceSimulatorControllerUI controller)
        {
            ...
            controller.OnGrip(IsActionInProgress(m_Simulator.gripAction));
            controller.OnTrigger(IsActionInProgress(m_Simulator.triggerAction));
            controller.OnMenu(IsActionInProgress(m_Simulator.menuAction));
            controller.OnPrimaryButton(IsActionInProgress(m_Simulator.primaryButtonAction));
            controller.OnSecondaryButton(IsActionInProgress(m_Simulator.secondaryButtonAction));
            controller.OnXAxisTranslatePerformed(IsActionInProgress(m_Simulator.keyboardXTranslateAction));
            controller.OnZAxisTranslatePerformed(IsActionInProgress(m_Simulator.keyboardZTranslateAction));
        }

        static bool IsActionInProgress(InputActionReference reference)
        {
            return reference != null && reference.action != null && reference.action.inProgress;
        }
